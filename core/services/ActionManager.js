...
export default class ActionManager {
  ...
  static applyCurrentPlayerAction(gameData, socket, action, playerId, params) {
    ...
    if (!action) {
      ...
      new AppError(socket, "Action not found for current player");
      return;
    }
    ...
    if (!PlayerManager.isPlayerActifInGame(currentPlayer)) {
      canDoAction = false;
      const msg = "player is not actif in game : cant do action";
      actionManagerLogger.error(msg);
      fileLogger.error(
        new Error(msg),
        "ActionManager.js  -->  applyCurrentPlayerAction()",
      );
      new AppError(socket, msg);
      return;
    }
    ...
    if (canDoAction && TypeManager.isDefined(playerId)) {
      ...
      const msg = "Cannot do action for current player";
      actionManagerLogger.error(msg);
      fileLogger.error(
        new Error(msg),
