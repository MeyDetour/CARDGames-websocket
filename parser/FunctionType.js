...
import Conditions from "../core/engine/Conditions.js";
...
export default class FunctionType extends TypeInterface{
    ...
    static splitLogical(str,gameData,params){
        ...
        if (str.startsWith("getPlayer(")){
            ...
            let value = Parser.translateInnerExpression(str,gameData,{...params, depth : params.depth+10})
            if (params.fileLogger) {
                params.fileLogger.log(Parser.getDepthIndentation(params.depth) + `Action is Get Player with value : ${value}   `);  
            }
            let player = PlayerManager.getPlayer(gameData,value)
            if (params.fileLogger) {
                params.fileLogger.log(Parser.getDepthIndentation(params.depth) + `FunctionType result: ${LoggerClass.pretty( player )}`);
            }
            return player
        } 
        return null
    ...
    static applyLogical(str,gameData,params){
        ...
        if (str === "allPlayersHasPlayed/endOfTour"){
            ...
            if (params.fileLogger) {
                params.fileLogger.log(Parser.getDepthIndentation(params.depth) + `FunctionType result: ${result}`);
            }
            return   result
        }
        const specialEvents = ["onChangeTour","startOfGame", "eachStartOfManche"]
        if (specialEvents.includes(str)){
            if (specialEvents.includes(params.eventEmited) && params.eventEmited === str )  {
                if (params.fileLogger) {
                    params.fileLogger.log(Parser.getDepthIndentation(params.depth) + `Action event matched emitted event: ${params.eventEmited}`);
                    params.fileLogger.log(Parser.getDepthIndentation(params.depth) + `FunctionType result: true`);
                }
                return true
            }
